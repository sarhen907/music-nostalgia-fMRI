...
if (len(sys.argv) < 2):
	print("\n\tusage: %s <input folder which contains field>\n" % sys.argv[0])
	sys.exit()
else:
	folder = sys.argv[1]

infile = os.path.join(folder,"fieldmap_mag.nii.gz")
#infile = os.path.join(folder,"coMPRAGE.nii.gz")
print(infile)
...
outfile = os.path.join(folder,"fieldmap_mag_brain.nii.gz")
betcommand = "bet %s %s -c %d %d %d -B -f .37 -o" % (infile,outfile,xcenter,ycenter,zcenter)
print( betcommand)
